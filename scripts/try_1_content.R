...
metadata <- metadata %>% select( title, content_id, show_type, category, keywords, audience )
...
rm(list_df_content, i, j, features, content_features_list, content_features_func)
...
# content_cluster contiene el cluster  al cual pertenece cada contenido
content_cluster <- cbind( content_id = row.names(features_matrix), cluster = km.res$cluster)

rm(features_matrix, km.res)


# # showtype  de 6 a 3 clases. 'gaming' 'pelicula' 'serie' ------------------------------------

content_showtype_audience <- metadata %>% select(content_id, show_type, audience)

for (i in 1:nrow(content_showtype_audience)) {
        content_showtype_audience$show_type[i] <- ifelse(content_showtype_audience$show_type[i] %in% c('Serie', 'TV', 'Rolling', 'Web', 'Series,Serie'),
                                                         'serie',
                                                         ifelse(content_showtype_audience$show_type[i] == "Película", 'pelicula', 'gaming')
                                                         )
}

for (i in 1:nrow(content_showtype_audience)) {
        content_showtype_audience$audience[i] <- switch(content_showtype_audience$audience[i],
                                                        "General" = 'general',
                                                        "Juvenil" = 'juvenil',
...
content_cluster <- content_cluster %>% as_tibble()
content_cluster$content_id <- content_cluster$content_id %>% as.numeric()
content_cluster$cluster <- content_cluster$cluster %>% as.numeric()

content_showtype_audience <- content_showtype_audience %>% 
        dplyr::distinct() 
...
content <- full_join(content_showtype_audience, content_cluster)

rm(content_cluster, content_showtype_audience)

save(content, file = paste(data_path, 'content_data.RData', sep = ""))
